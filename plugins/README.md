...
| [yt-toolkit](./yt-toolkit/) | Transcript-driven YouTube editing tools: dead-air cuts, chapters, retention diagnosis, and hook scoring | **Skills:** `yt-edit`, `yt-chapters`, `yt-retention`, `yt-hook` - Python scripts over a shared `.srt`/`.vtt`/whisper-`.json` transcript parser and a Studio retention-CSV reader |
