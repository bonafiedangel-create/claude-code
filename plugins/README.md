...
| [youtube-agent](./youtube-agent/) | Eleven skills that run a YouTube channel end to end, adapted from Jake Schincariol's `youtube-agent-skill` (MIT) | **Skills:** `yt-script`, `yt-package`, `yt-edit`, `yt-chapters`, `yt-retention`, `yt-viral`, `yt-shorts`, `yt-seo`, `yt-comment`, `yt-plan`, `yt-audit` - six run Python tools over a shared transcript parser and hook-formula library |
