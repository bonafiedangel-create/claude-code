...
A tool lives in this skill's `scripts/` folder and it actually runs. Use it. Do not eyeball the hook.

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/yt-script/scripts/hookscore.py" hooks.txt          # rank your hook options
python3 "${CLAUDE_PLUGIN_ROOT}/skills/yt-script/scripts/hookscore.py" --hook "one line"  # score a single one
```
...
1. **Hook.** Write FIVE against [the 21 formulas](../../lib/hooks.json), run them through
   `hookscore.py`, keep the top two, and show the user both with their scores. Never hand over one
   hook.
