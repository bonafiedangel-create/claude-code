...
1. **The last ten titles, as a set.** Read them as a list, the way the channel page shows them. Do
   they promise different things? Run them through
   `${CLAUDE_PLUGIN_ROOT}/skills/yt-package/scripts/title.py`. A channel where every title is the
   same shape has a format problem, not a title problem.
...
3. **The first fifteen seconds of the three most recent.** Transcribe them and score with
   `${CLAUDE_PLUGIN_ROOT}/skills/yt-script/scripts/hookscore.py`. This is where most channels lose.
